...
class EphemeralCouchDbInstance(implicit ec: ExecutionContext) extends CouchDbInstance {
  ...
  val (process, url) = {
    ...
    val uriFile: Path = Await.result(uriFileCreated(baseDir), 60.seconds)
    val url = Files.readAllLines(uriFile).get(0)
    (process, new URL(url))
  }
